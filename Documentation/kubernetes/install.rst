...
change the `ConfigMap` inside of it with the necessary changes based on the
...
If you are running more than one nodes simply add it to the list of endpoints.
...
#. Create a kubernetes secret with certificate authority file in kubernetes;
    ...
    #. Create a file with the name ``ca.crt`` that contains the certificate
       authority inside of it;
...
If etcd is running with client to server authentication, you need make more
...
#. Create a kubernetes secret with certificate authority file in kubernetes.
    ...
    #. Create a file with the name ``client.key`` that contains the client key
       inside of it;

    #. Create a file with the name ``client.crt`` that contains the client
       certificate inside of it;
...
.. note::
    ...
    If you have set up the secret you might see the error
...
* ``sidecar-http-proxy`` - Runs the HTTP proxy as a sidecar;
...
* ``legacy-host-allows-world`` - If true, the policy with the entity
  ``reserverd:host`` allows traffic from ``world``. If false, the policy needs
  to explicitly have the entity ``reserverd:world`` to allow traffic from
  ``world``.


Any changes that you perform in the Cilium `ConfigMap` and in
``cilium-etcd-secrets`` ``Secret`` will require you to delete any existing
